...
function logError(msg: string): void {
  state.errors.unshift({ ts: Date.now(), msg: msg.slice(0, 200) });
  if (state.errors.length > 10) state.errors.length = 10;
...
function snapshot(): unknown {
  const mem = fmtMem();
  const uptimeS = Math.round((performance.now() - state.startedAt) / 1000);
  const tiles = state.tilesLoaded
    ? { loaded: state.tilesLoaded, avgMs: Math.round(state.tileTotalMs / state.tilesLoaded) }
    : null;
  const snaps = state.snapsTotal
    ? { total: state.snapsTotal, avgMicros: Math.round((state.snapTotalMs / state.snapsTotal) * 1000) }
    : null;
  const endpoints: Record<string, unknown> = {};
  for (const [k, s] of Object.entries(state.endpoints)) {
    endpoints[k] = {
      count: s.count,
...
function renderPanel(): void {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
      <strong style="font-size:11px">DEBUG · ${s.uptimeS}s · ${s.online ? '🟢' : '🔴'}</strong>
      <div>
...
export function initDebug(): void {
  ...
  function frame() {
    ...
    if (now - lastFpsT >= 1000) {
      state.fps = Math.round((frames * 1000) / (now - lastFpsT));
      frames = 0;
      lastFpsT = now;
    }
...
export function trackTile(ms: number): void {
  state.tilesLoaded += 1;
  state.tileTotalMs += ms;
}

export function trackSnap(ms: number): void {
  state.snapsTotal += 1;
  state.snapTotalMs += ms;
}
